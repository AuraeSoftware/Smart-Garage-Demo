...
def startup_event():
    ...
    try:
        with engine.connect() as conn:
            ...
            conn.execute(text("ALTER TABLE customers ADD COLUMN IF NOT EXISTS username VARCHAR(100)"))
            conn.execute(text("ALTER TABLE customers ADD COLUMN IF NOT EXISTS password_hash VARCHAR(200)"))
            conn.execute(text("ALTER TABLE customers ADD COLUMN IF NOT EXISTS email VARCHAR(200)"))
            conn.execute(text("ALTER TABLE customers ADD COLUMN IF NOT EXISTS total_spend FLOAT DEFAULT 0"))
            conn.execute(text("ALTER TABLE customers ADD COLUMN IF NOT EXISTS last_visit VARCHAR(50)"))
            conn.execute(text("ALTER TABLE customers ADD COLUMN IF NOT EXISTS joined_at TIMESTAMP DEFAULT now()"))
            conn.execute(text("ALTER TABLE customers ADD COLUMN IF NOT EXISTS notes TEXT DEFAULT ''"))
            conn.execute(text("ALTER TABLE customers ADD COLUMN IF NOT EXISTS coupons_redeemed INTEGER DEFAULT 0"))
            conn.execute(text("ALTER TABLE customers ADD COLUMN IF NOT EXISTS last_coupon_used TIMESTAMP"))
            conn.execute(text("ALTER TABLE customers ADD COLUMN IF NOT EXISTS visits JSON"))
            conn.execute(text("ALTER TABLE customers ADD COLUMN IF NOT EXISTS coupon_history JSON"))
            conn.commit()
    except Exception as e:
